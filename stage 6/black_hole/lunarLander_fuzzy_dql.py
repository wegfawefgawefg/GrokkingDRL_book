...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, inputShape, fc1Size, fc2Size, numOutputs):
        super().__init__()
        ...
        self.fc3 = nn.Linear(fc2Size, numOutputs * 2)
...
class DQAgent():
    ...
    def chooseActions(self, observation):
        ...
        actionValues = []
        for i in range(0, self.numActions, 2):
            norm = torch.distributions.normal.Normal(policy[i], policy[i+1])
            actionValue = norm.rsample()
